...
if (typeof libcurl !== 'undefined') {
    libcurl.init({ wisp: WISP_SERVER_URL });
} else {
...
const REWRITER_SOURCE_PART_TWO = `
    ...
    if (typeof HTMLFormElement !== 'undefined') {
        ...
        if (desc) Object.defineProperty(HTMLFormElement.prototype, 'action', { get: function() { return unproxyUrl(desc.get.call(this)); }, set: function(val) { desc.set.call(this, rewriteUrl(val)); } });
    }
`;
// ==========================================
// 
...
self.addEventListener('fetch', (event) => {
    ...
    if (requestUrl.pathname.startsWith(PROXY_PREFIX)) {
        ...
        try {
            ...
            event.respondWith(
                libcurl.fetch(targetUrl.href, fetchOptions).then(async (response) => {
            ...
            event.respondWith(new Response(generateErrorPage(err.message, 400), { status: 400, headers: { 'Content-Type': 'text/html' } }));
        }
        return;
    }
// ==========================================
